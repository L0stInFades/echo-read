...
启动器图标按自适应图标规范重做：**实心开卷剪影 + 上方两道声弧**，背景是取自新调色板的
`primary T30 → T40 → tertiary T40` 渐变，另配 `monochrome` 层支持 Android 13+ 的主题图标。
放弃了 0.1.x 的细描边画法 —— 1.8pt 描边在 48dp 启动器与 24dp 通知栏会糊成灰团，而 Expressive 要的正是单一焦点的大胆块面。
图形外接框完整落在 66% 安全区内，定稿前在 320px / 48px 两档实际渲染比对过四种构图。
