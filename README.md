...
- **离线书架**：封面网格、继续阅读、书名/作者搜索、最近阅读/最近导入/书名排序；点封面打开，长按删除；支持多选导入与拖放 TXT / EPUB。
...
`android/` 目录是一套独立的 **Kotlin 原生安卓应用**（Jetpack Compose + Room + Media3），功能与网页版对齐：任意字点读、OpenRouter/OpenAI 兼容 TTS 与系统语音兜底、锁屏与通知栏控制、后台连播、断网自愈、音频缓存、TXT/EPUB 导入。

当前为 **0.2.0-exp（实验版）**：界面整体换用 **Material 3 Expressive**（配色由 Google 的色彩算法按 seed 生成，动效把 M3 的动效 token 与自研 CA 管线融合，五档弹簧与 M3 数值完全一致）；新增**可配置翻页手势**（左右/上下滑、点击热区位置与范围、灵敏度，带实时预览）；导入改为**应用内全盘扫描**（自动找出本机所有 TXT/EPUB，多选批量入库）；启动器图标按自适应图标规范重做。改动较激进，更新提示可直接叉掉。构建、设计系统与使用见 [android/README.md](android/README.md)。
...
应用首页就是书架。右上角问号也可随时打开「怎么用」。

1. 部署 `dist/` 到任意静态托管（HTTPS 或 localhost 才能启用 PWA 与音频自动播放）。
2. **配声音**：打开应用 → 齿轮 → 填入 OpenRouter API Key（[在此创建](https://openrouter.ai/settings/keys)），点「获取在线模型」后选择模型与音色（默认 Kokoro 超低价中文音色，Fish S2.1 有免费档可先试听），点「试听测试」验证。没有 Key 时切到「系统语音」，用浏览器自带朗读。
3. **往书架放书**：点「导入」选择 TXT / EPUB（可多选）；电脑上也可以把文件拖进页面。没有书时点「先听示例」会写入内置《深夜书屋》。
4. **从书架读**：点封面打开；顶部「继续阅读」回到上次那本；搜索框按书名/作者过滤，排序可选最近阅读、最近导入或书名。长按封面可继续阅读或从书架删除。
5. **点读**：打开书籍后，**轻点正文任意位置**，AI 即从该字开始朗读。底栏可暂停、切章、调倍速；月亮按钮是睡眠定时（15–90 分钟或播完本章）。左上角返回书架。
