# Lector · AI 听书（Android）

> **0.3.0-exp** —— 实验版。应用由 EchoRead 更名为 **Lector**，全新标志与图标；0.2.x 起 UI 整体换用 Material 3 Expressive、新增可配置翻页手势、导入改为应用内全盘扫描。改动较激进，更新卡片可以直接叉掉（按版本永久忽略）。

原生 Kotlin / Jetpack Compose 安卓应用：**导入 TXT / EPUB，轻点正文任意字，AI 便从那里开始朗读**。自填 API Key、自传书籍，无账号、无后端。
...
> 章节数据模型与 TXT 分章的部分思路参考了开源阅读器 [read-cat](https://github.com/read-cat/read-cat)，面向 AI TTS 场景独立全新实现，经逐行比对未使用其任何代码，特此致谢其思路启发。

## 仓库结构

```
android/             Android 工程（Gradle 根目录，用 Android Studio 打开它）
android/update.json  应用内更新清单。路径已写死在所有已发布客户端里，不能移动或改名
android/release.sh   发布脚本：签名打包 → 写清单 → 提交推送 → GitHub Release
design/lector/       品牌资产：标志、字标、应用图标源文件与 Android 矢量图标
```

包名仍是更名前的 `app.echoread`，Kotlin 包路径、数据库与本地存储的名字也一并保留：改任何一个都会让老用户无法覆盖更新或丢数据。
...
> 1.5.0-alpha18 是仍能在 AGP 8.x 上构建的最后一个 alpha：alpha19 起 `minCompileSdk=37 / minAGP=9.1.0`。同理 compose-bom 不能升到 2026.08.00（compose ui 1.12.0 有同样的要求）。升级前请先读 `android/gradle/libs.versions.toml` 里的注释。
- minSdk 26 · targetSdk/compileSdk 36 · AGP 8.13 · Gradle 8.14（wrapper 自带）

```
android/app/src/main/kotlin/app/echoread/
  ...
  data/   Db(Room) SettingsStore(SharedPreferences+JSON) ChapterCache(LRU 4 章) Library(导入归一化/进度)
          BookScanner(全盘扫描：整机权限 / SAF 目录 / 媒体库 三条路径合并去重) Updater(应用内更新)
  ...
  ui/motion/  MotionTokens(8 档弹簧，6 档等于 M3 token) MotionDriver(统一驱动器)
...
android/baselineprofile/   Baseline Profile 生成器 + Macrobenchmark（启动、翻页帧耗时）
```

模块方向严格单向：`core ← data ← tts ← ui`。任一章节在内存中只有一份规范纯文本，段落/片段/高亮皆为它的偏移区间 `Range(start, end)`。
...
运行时按「种子色 × 风格 × 明暗 × 对比度」现算（`ColorSchemes.kt`），用户可在阅读样式里换种子色与风格，或直接跟随壁纸。
...
`EchoMotion` 共 8 档，取值全部来自 material3 1.5.0-alpha18 里
`StandardMotionTokens` / `ExpressiveMotionTokens` 的实际字节码常量，用 `response = 2π/√k` 换算到我们的
`(response, damping)` 参数化。八档里**六档与 Google 的 token 数值完全相等**：
...
标志是**中文引号「 + 一个方块字**，即正文里那句被高亮、正在朗读的字；不用书本、声波、麦克风。
品牌色 `#7C9BFF` 就是配色系统的种子色，墨色 `#111F55` 是同色相的深调；字标用 Outfit Bold，已转轮廓。
源文件、组合规范与 Android 矢量图标见 `design/lector/`。

启动器图标：背景层平铺品牌色，前景为墨色引号与白色方块，另配 `monochrome` 层支持 Android 13+ 的主题图标，通知栏小图标同一套几何。
两块几何在 200 单位设计盒内按 1.08 缩放居中于 512 视口，最远着墨点距中心约 147，圆形 / 方圆 / 圆角方遮罩下均无裁切。
启动画面（API 31+）给图标垫了同色圆底，否则白色方块会消失在浅色启动底上。

## 构建

需要 JDK 17+（Gradle 8.14 最高支持到 JDK 24）与 Android SDK（`compileSdk 36`）。用 Android Studio 打开 `android/` 目录，或命令行：

```bash
cd android
./gradlew :app:assembleDebug          # 调试包 → app/build/outputs/apk/debug/app-debug.apk
./gradlew :app:testDebugUnitTest      # 单元测试（解析/分段/音色目录/请求体/退避/EPUB/配色/动效/手势）
...
> **待办**：0.2.0 改版引入了新的启动/导入路径与 M3 组件，仓库里的 Baseline Profile 尚未重新生成
> （需要连一台 API 33+ 真机或模拟器跑 `:app:generateReleaseBaselineProfile`）。现有 profile 仍覆盖未变的启动主路径，
> 只是新代码暂时享受不到预编译，属性能优化而非正确性问题。

- `android/app/src/release/generated/baselineProfiles/` 是提交在库里的 Baseline/Startup Profile，随 release 包一起打进 APK（`profileinstaller` 安装即生效，冷启动与首屏免 JIT）。
- 改了启动/阅读路径的代码后重新生成（需连一台 API 33+ 的设备或模拟器）：`ANDROID_SERIAL=<serial> ./gradlew :app:generateReleaseBaselineProfile`
- 压测：`ANDROID_SERIAL=<serial> ./gradlew :baselineprofile:connectedBenchmarkReleaseAndroidTest`（`StartupBenchmark` 对比有无 profile 的冷启动；`ReaderScrollBenchmark` 翻页 `FrameTimingMetric`）。结果在 `android/baselineprofile/build/outputs/connected_android_test_additional_output/`。
...
```bash
cd android
./release.sh 0.3.1-exp 15 "更新说明"   # 打正式签名包 → 写 update.json → 提交推送 → gh release 上传 APK
```

正式签名读取 `android/keystore.properties`（`storeFile / storePassword / keyAlias / keyPassword`，已 gitignore）；没有该文件时回退 debug 签名，仅供本地验证。版本号带 `-exp` 时清单会标记为实验版，更新卡片带风险提示。

## 使用

1. 打开应用 → 右上角齿轮 → 填入 OpenRouter API Key（[在此创建](https://openrouter.ai/settings/keys)），点「获取在线模型」选择模型与音色，点「试听」验证；没有 Key 时切到「系统语音」。
2. 底部「导入书籍」自动扫描本机 TXT / EPUB，勾选后批量入库；没有书时点「先听示例」。
3. 打开书籍后轻点正文任意位置开始朗读；底栏可暂停、切章、调倍速、睡眠定时；锁屏与通知栏可控制播放。

## 许可

本项目采用 **[The JSON License](https://www.json.org/license.html)**（SPDX：`JSON`），全文见 [LICENSE](LICENSE)。它就是 MIT 许可证外加一句：

> The Software shall be used for Good, not Evil.

其余与 MIT 相同：可自由使用、复制、修改、合并、发布、分发、再许可与出售，只需保留版权声明与许可声明；软件按「原样」提供，不附带任何担保。

> 因为那句「只可为善」，这份许可证未获 OSI 认证，FSF 与 Debian 也把它归为非自由许可证。介意的话请按此评估。
